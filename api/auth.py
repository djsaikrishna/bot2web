...
chk_bp = Blueprint('chk_bp', __name__)
last = 1
...
def chk_route():
    ...
    while rt < 3:
        try:
            ...
            if match:
                otuput = f'<br>CC: {cc}<br>Result: {match.group(1)}<br>Gate: Raven BT Auth<br>Retries: {rt}<br>Time: {total}'
            elif not match:
                otuput = f'<br>CC: {cc}<br>Result: {result}<br>Gate: Raven BT Auth<br>Retries: {rt}<br>Time: {total}<br>'
            return otuput
